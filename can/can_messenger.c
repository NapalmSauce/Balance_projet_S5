...
int main(int argc, char *argv[]) //char **argv)
{
  ...
  {
    perror(stderr,argv[0]);
  }
  
  if(S_ISCHR(theinput.st_mode))
  {
    if(tcsetattr(STDIN_FILENO, TCSANOW, &PortSettingsOld))
    {
      perror("can_messenger: tcsetattr()");
    }
  }
...
int processus_pere(void)
{
  ...
  //   frame.can_dlc = 7;		// nombre d'octets de données
    ...
    {
    case 'p':
      frame_out.can_id = 0x142;  	// identifiant CAN, exemple: 247 = 0x0F7
      break;
    case 'm':
      frame_out.can_id = 0x141;  	// identifiant CAN, exemple: 247 = 0x0F7
      break;
    case 's':
      frame_out.can_id = 0x143;  	// identifiant CAN, exemple: 247 = 0x0F7
      break;
    case 'q':
      write(pipefd[1],"q",1);
      return 0;
...
int processus_fils(void)
{
 ...
 //     write(STDERR_FILENO,'\n',stderr);
    ...
    //Postion char
    else if(frame_in.can_id == 0x161)
    {
      if(frame_in.data[0] == '3')
      {
